...
exports.getWeeklySales = async (req, res) => {
  try {
    const sales = await Report.getWeeklySales(); // ✅ gunakan Report, bukan Order
    res.status(200).json(sales);
  } catch (error) {
    console.error("Gagal mengambil data penjualan mingguan:", error);
    res.status(500).json({ error: "Gagal mengambil data penjualan mingguan" });
  }
};
